...
public class PlayBill {
	...
	public PlayBill() throws IOException, ParseException {
        URL src_url = new URL("http://localhost:8080/PA3/serve?file=Playbill.txt");
        ...
        while ((line = in.readLine()) != null) {
        	if(line != null && !line.isEmpty()) {
	        	...
	        	if (line_split.length == 4) {
	        		Review review = new Review("Playbill", line_split[0].trim(), line_split[1].trim(), date, line_split[3].trim());
	        		reviews.add(review);
	        	}else if (line_split.length == 5) {
	        		Review review = new Review("Playbill", line_split[0].trim(), line_split[1].trim(), date, line_split[3].trim(), line_split[4].trim());
	        		reviews.add(review);
	        	}
        	}
        }
