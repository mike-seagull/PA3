...
public class Soap {
	...
	public Soap() throws IOException, ParseException {
		String[] sources = {"Broadway.com", "NYTimes", "Playbill"};
		for (String source : sources) {
	        URL src_url = new URL("http://localhost:8080/PA3/serve?file="+source+".txt");
	        ...
	        while ((line = in.readLine()) != null) {
	        	if(line != null && !line.isEmpty()) {
		        	...
		        	if (line_split.length == 4) {
		        		Review review = new Review(source, line_split[0].trim(), line_split[1].trim(), date, line_split[3].trim());
		        		reviews.add(review);
		        	}else if (line_split.length == 5) {
		        		Review review = new Review(source, line_split[0].trim(), line_split[1].trim(), date, line_split[3].trim(), line_split[4].trim());
		        		reviews.add(review);
		        	}
	        	}
	        }
